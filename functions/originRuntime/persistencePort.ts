...
// Swap the Base44Adapter for a Node/Postgres adapter, a WASM in-memory adapter, etc.
// and the entire stage pipeline runs unchanged. This is what makes the runtime portable.
...
export interface PersistencePort {
  ...
  recordStageObservation(unit: any, stageName: string): Promise<void>;
...
export class Base44Adapter implements PersistencePort {
  ...
  async ensureContainer(source_name: string, author_id: string, author_domain_id: string, hol_context: string): Promise<string> {
    ...
    await this.base44.asServiceRole.entities.Molecule.create({
      molecule_id: containerId, canonicalHash: hash,
      lexical_content: source_name, molecule_type: "container",
      current_state: "EXPLORED", constitutional_status: "active",
      access_tier: "open", weight_class: "operational", is_foundational: false,
  ...
  async persistMolecule(unit: any): Promise<void> {
    await this.base44.asServiceRole.entities.Molecule.create({
      ...
      lexical_content: unit.normalised_text,
      molecule_type: unit.molecule_type,
      current_state: unit.current_state,
      constitutional_status: unit.constitutional_status,
      access_tier: unit.access_tier,
      weight_class: unit.weight_class,
      is_foundational: false,
      kCv_o: unit.kCv_o, kCv_u: unit.kCv_u,
      kCv_v_score: 0, kCv_v_quality: "UNVERIFIED",
      kCv_i_score: 0, kCv_i_status: "UNOBSERVED",
      kCv_r_score: 0, kCv_r_status: "NEW",
      kCv_rank: unit.kCv_rank,
      capture_confidence: String(unit.capture_confidence),
      observation_density: 0, reuse_count: 0,
      parent_molecule_ids: unit.parent_molecule_ids,
      lineage_types: unit.lineage_types,
      lineage_certainty: "0.9",
      scope_definition: unit.source_name + "::" + unit.source_name,
      section: unit.source_name,
      source_name: unit.source_name,
      hol_context: unit.hol_context,
      author_domain_id: unit.author_domain_id,
      vsid: unit.author_id,
      constitution_version: "v1.0",
      state_since: unit.state_since,
    });
  }

  async recordStageObservation(unit: any, stageName: string): Promise<void> {
    ...
    await this.base44.asServiceRole.entities.JournalObservation.create({
      observation_id: oid,
      molecule_id: unit.molecule_id || "pending",
      canonicalHash: unit.canonicalHash || "",
      observation_type: "pipeline_stage:" + stageName.toLowerCase(),
      polarity: 1,
      conflict_flag: false,
  ...
  async recordConstitutionalFact(fact: any): Promise<void> {
    ...
    await this.base44.asServiceRole.entities.JournalFact.create({
      ...
      weight_class: fact.weight_class,
      polarity: 1,
      from_state: fact.from_state,
      to_state: fact.to_state,
      actor_id: fact.actor_id || "runtime",
      actor_domain_id: fact.actor_domain_id || "runtime",
      actor_trust_score: 1.0,
